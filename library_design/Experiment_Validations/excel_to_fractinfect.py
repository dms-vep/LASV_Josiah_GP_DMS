...
def parse_args():
    ...
    parser.add_argument('--start_row', type=int, help="Row of excel file "
                        "where the plate column headers are, in 0-indexing. "
                        "The default for luciferase plates is 22. For GFP "
                        "plates, the value should likely be 30.",
                        default=22)
...
def main():
    ...
    for plate in sheet_data:
        ...
        sample_dtype = plate_map['SampleNum'].dtype
        if sample_dtype in [np.dtype('float64'), np.dtype('float32')]:
            sample_nums = plate_map['SampleNum'].astype(int).tolist()
        else:
