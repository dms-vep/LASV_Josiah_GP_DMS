...
rule get_protein_sequences_with_outgroup:
    """
    This rule runs EMBOSS getorf to extract 
    amino acid sequences for GPC segment. 
    """
    input:
        sequences = config["Nucleotide_sequences_with_outgroup"],
    params:
        config["Min_ORF_threshold"],
    output:
        config["Protein_sequences_with_outgroup"],
    shell:
        # The '-sequence' flag signals for the input file
        # while the '-outseq' file signals for the output 
        # file. The '-find 1' flag means the amino acid 
...
rule get_codon_sequences_with_outgroup:
    """
    This rule runs EMBOSS getorf to extract 
    codon sequences for GPC segment. 
    """
    ...
    params:
        config["Min_ORF_threshold"],
    output:
        config["Codon_sequences_with_outgroup"],
    shell:
        # The '-sequence' flag signals for the input file
        # while the '-outseq' file signals for the output 
        # file. The '-find 3' flag means the nucleotide 
...
rule check_number_ORFs_found_with_outgroup:
    """
    This rule checks both the amino acid
    and codon fastas to verify there is a 
    one-to-one mapping from S segment nucleotide
    ...
    """
    input:
        nucleotide = config["Nucleotide_sequences_with_outgroup"],
        protein = config["Protein_sequences_with_outgroup"],
        codon = config["Codon_sequences_with_outgroup"],
    output:
        config["Sequence_verification_log_with_outgroup"]
    script:
        "../Scripts/extracted_sequence_verification.py"